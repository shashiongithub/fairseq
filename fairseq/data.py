...
def load_raw_text_dataset(path, load_splits, src=None, dst=None):
    ...
    for split in load_splits:
        ...
        src_indexedrawdataset = IndexedRawTextDataset(src_path, src_dict, is_dst=False)
        
        dst_path = os.path.join(path, '{}.{}'.format(split, dst))
        dst_indexedrawdataset = IndexedRawTextDataset(dst_path, dst_dict, is_dst=True, src_oov_words_list=src_indexedrawdataset.oov_words_list)
        
        dataset.splits[split] = LanguagePairDataset(
            src_indexedrawdataset, dst_indexedrawdataset,
...
class LanguagePairDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        res['source'] = self.src[i]["tokens"].long() - 1
        res['source_extended'] = self.src[i]["tokens_extended"].long() - 1
        res['source_oov'] = self.src[i]["oov_words"]
        if self.dst:
            res['target'] = self.dst[i]["tokens"].long() - 1
            res['target_extended'] = self.dst[i]["tokens_extended"].long() - 1
    ...
    def collate(samples, pad_idx, eos_idx, has_target=True):
        ...
        src_tokens_extended = merge('source_extended', left_pad=LanguagePairDataset.LEFT_PAD_SOURCE)
        src_oov_words = [s['source_oov'] for s in samples]
        ...
        src_tokens_extended = src_tokens_extended.index_select(0, sort_order)
        src_oov_words = [src_oov_words[idx] for idx in sort_order]        
        max_src_oov_words = max([len(oov_wordlist) for oov_wordlist in src_oov_words])
        ...
        if has_target:
            ...
            target_extended = merge('target_extended', left_pad=LanguagePairDataset.LEFT_PAD_TARGET)
            ...
            target_extended = target_extended.index_select(0, sort_order)
        ...
        return {
            ...
            'copy_mechanism': {
                'src_tokens_extended':  src_tokens_extended,
                'src_oov_words': src_oov_words,
                'max_src_oov_words': max_src_oov_words,
...
def _valid_size(src_size, dst_size, max_positions, oov_size, max_oov_size=20):
...
def _make_batches(src, dst, indices, max_tokens, max_sentences, max_positions, 
    ...
    for idx in map(int, indices):
        ...
        oov_size = src.oov_sizes[idx]
        
        if not _valid_size(src_size, dst_size, max_positions, oov_size, max_oov_size=20):
            if ignore_invalid_inputs:
                ignored.append(idx)
                continue
            raise Exception((
                "Sample #{} has size (src={}, dst={}, oov={}) but max size is {} and max_oov is 20."
                " Skip this example with --skip-invalid-size-inputs-valid-test"
            ).format(idx, src_size, dst_size, oov_size, max_positions))
...
def shuffled_batches_by_size(src, dst, max_tokens=None, max_sentences=None,
    ...
    # sort by sizes: target token sizes
    indices = indices[np.argsort(dst.sizes[indices], kind='mergesort')]
    # sort by sizes: number of oov words in the source
    indices = indices[np.argsort(src.oov_sizes[indices], kind='mergesort')]
    # sort by sizes: source token sizes
