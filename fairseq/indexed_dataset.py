...
class IndexedRawTextDataset(IndexedDataset):
    ...
    def __init__(self, path, dictionary, is_dst=False, src_oov_words_list=None):
        self.tokens_list = []
        self.tokens_list_extended_vocab = []
        self.oov_words_list = []
        ...
        self.read_data(path, dictionary, is_dst=is_dst, src_oov_words_list=src_oov_words_list)
    ...
    def read_data(self, path, dictionary, is_dst=False, src_oov_words_list=None):
        ...
        # print("All OOV: ", len(self.oov_words_list))
        # oov_lens = [len(item) for item in self.oov_words_list]
        # oov_dict = {}
        # for item in oov_lens:
        #     if item not in oov_dict:
        #         oov_dict[item] = 1
        #     else:
        #         oov_dict[item] += 1
        # keys = oov_dict.keys()
        ...
        #     print(i, oov_dict[i])
        # print("")
        # print("average: ", float(sum(oov_lens))/len(oov_lens))
        # # oov_lens.sort()
        # # print("median: ", oov_lens[len(oov_lens)/2])
                
    def __getitem__(self, i):
        self.check_index(i)        
        res = {"tokens": self.tokens_list[i],
               "tokens_extended": self.tokens_list_extended_vocab[i],
               "oov_words":self.oov_words_list[i]}
        return res
